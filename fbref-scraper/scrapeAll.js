...
module.exports = async (req, res) => {
  ...
  // After getting the data, store it in MongoDB instead of the cache
  try {
    await client.connect();
    const collection = client.db("jerome").collection("players"); 
    const promises = data.map((playerData) => {
      return collection.updateOne({name: playerData.name}, {$set: playerData}, {upsert: true});
    });
    await Promise.all(promises);
    res.send({ status: 'Data scraped and stored in DB', playerNames: data.map(player => player.name) });
  } catch (err) {
    console.error(err);
    res.status(500).send({ status: 'Failed to store data in DB' });
  } finally {
    await client.close();
  }
};